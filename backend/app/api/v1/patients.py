...
def ensure_demo_caregiver_access(user_id: str) -> None:
    """Development-only bootstrap for legacy demo caregiver accounts."""
    ...
    try:
        ...
        if profile:
            ...
            metadata = (auth_user.user.user_metadata or {}) if auth_user and auth_user.user else {}
            role = metadata.get("system_role") or "professional_caregiver"
            full_name = metadata.get("full_name") or "MediQ Caregiver"
        if role not in CAREGIVER_ROLES:
            return
        admin.table("profiles").upsert({"id": user_id, "full_name": full_name, "system_role": role}).execute()
        ...
        for patient in patients_resp.data or []:
            admin.table("patient_memberships").upsert({
                "user_id": user_id, "patient_id": patient["id"], "assigned_role": role
            }, on_conflict="user_id,patient_id").execute()
    except Exception:
        return

@router.get("", response_model=list[PatientResponse])
async def get_patients(user_id: str = Depends(get_current_user), client=Depends(get_user_supabase_client)):
...
async def get_patient(patient_id: UUID, user_id: str = Depends(get_current_user), client=Depends(get_user_supabase_client)):
    ensure_demo_caregiver_access(user_id)
    try:
...
async def update_patient(patient_id: UUID, payload: dict, user_id: str = Depends(get_current_user), client=Depends(get_user_supabase_client)):
    ensure_demo_caregiver_access(user_id)
    allowed = {}
    if isinstance(payload.get("primary_diagnosis"), str):
        allowed["primary_diagnosis"] = payload["primary_diagnosis"].strip()
    if isinstance(payload.get("baseline_conditions"), dict):
...
async def get_patient_observations(patient_id: UUID, user_id: str = Depends(get_current_user), client=Depends(get_user_supabase_client)):
    ensure_demo_caregiver_access(user_id)
    try:
...
async def create_patient_handover(patient_id: UUID, user_id: str = Depends(get_current_user), client=Depends(get_user_supabase_client)):
    ensure_demo_caregiver_access(user_id)
    try:
        patient_check = client.table("patients").select("id, primary_diagnosis").eq("id", str(patient_id)).execute()
        if not patient_check.data:
    ...
    try:
        insert_resp = client.table("handover_summaries").insert({
            "patient_id": str(patient_id), "created_by": user_id,
...
async def get_patient_handovers(patient_id: UUID, user_id: str = Depends(get_current_user), client=Depends(get_user_supabase_client)):
    ensure_demo_caregiver_access(user_id)
    try:
