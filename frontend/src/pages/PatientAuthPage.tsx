...
export function PatientAuthPage({ initialMode = 'signin' }: AuthPageProps) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (mode === 'signin') {
      ...
      if (data?.user) {
        const userMetaRole = data.user.user_metadata?.system_role;
        ...
        const effectiveRole = profile?.system_role || userMetaRole;

        if (effectiveRole !== 'patient') {
          await supabase.auth.signOut();
          setError('This account is registered as a Care Team account. Please use the Care Team Portal.');
          setLoading(false);
          updateSpeech('pulse', 'Access denied. Incorrect portal.');
          return;
        }
      ...
      if (signUpData.session) {
        updateSpeech('shield', 'Registration successful! Opening patient workspace...');
        setLoading(false);
        navigate('/dashboard/patient', { replace: true });
        return;
      } else {
          ...
          <section className="lg:col-span-6 bg-white p-7 sm:p-10 lg:p-12 flex flex-col justify-between order-2 lg:order-1" data-purpose="auth-form-panel">
            <div>
              <h2 className="text-2xl sm:text-3xl font-heading font-bold text-slate-900 tracking-tight" id="form-heading">
                {mode === 'signin' ? 'Patient Sign In' : 'Patient Registration'}
              </h2>
              <p className="text-sm text-slate-500 mt-1">Manage your care, appointments, and health records securely.</p>
              ...
              <div className="mt-6 p-1 bg-slate-100 rounded-xl flex items-center border border-slate-200/70 text-xs font-semibold relative">
                <button 
                  type="button"
                  onClick={() => { setMode('signin'); updateSpeech('user', 'Welcome back to your patient portal.'); }}
                  className={cn(
                ...
                <button 
                  type="button"
                  onClick={() => { setMode('register'); updateSpeech('user', 'Setting up your secure patient record.'); }}
                  className={cn(
                    "flex-1 py-2.5 rounded-lg transition-all text-center cursor-pointer font-medium",
              ...
              <form onSubmit={handleAuth} className="mt-6 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative rounded-xl shadow-sm">
                      ...
                      <input 
                        ...
                        onFocus={() => {
                          updateSpeech('user', 'Setting up patient identity record.');
                    ...
                    <input 
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value.includes('@') && e.target.value.includes('.')) {
                          updateSpeech('check', 'Email format recognized. Please proceed with your secure password.');
                        } else if (e.target.value.length > 4) {
                          updateSpeech('pulse', 'Typing detected... validating identity record.');
                        }
                      }}
                      onFocus={() => {
                        updateSpeech('search', 'Verifying email address...');
                        setIsPortraitFocused(true);
                      }}
                      onBlur={() => setIsPortraitFocused(false)}
                    />
                ...
                {mode === 'register' && (
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5" htmlFor="dob-input">
                        DATE OF BIRTH
                      </label>
                      <input 
                        className="block w-full px-4 py-3 bg-slate-50/80 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-mediq-blue focus:border-mediq-blue transition-all font-sans" 
                        id="dob-input" 
                        required 
                        type="date" 
                        value={dob}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1.5" htmlFor="phone-input">
                        PHONE NUMBER
                      </label>
                      <input 
                        className="block w-full px-4 py-3 bg-slate-50/80 border border-slate-200 rounded-xl text-sm text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-mediq-blue focus:border-mediq-blue transition-all font-sans" 
                ...
                {mode === 'register' && (
                  <div className="flex items-center gap-2.5 mt-2">
                    <input 
                      id="terms-checkbox"
                      className="w-4 h-4 rounded text-mediq-blue border-slate-300 focus:ring-mediq-blue transition-all cursor-pointer" 
                      type="checkbox" 
                      checked={acceptTerms}
                      onChange={(e) => setAcceptTerms(e.target.checked)}
                      required
                    />
                    <label htmlFor="terms-checkbox" className="text-xs text-slate-600 font-medium cursor-pointer select-none">
                      I accept the <a href="#" className="text-mediq-blue hover:underline">Terms of Service</a> & <a href="#" className="text-mediq-blue hover:underline">Privacy Policy</a>
                    </label>
                  </div>
                )}
                ...
                >
                  {loading ? (
                    ...
                    <span>{mode === 'signin' ? 'Access Patient Portal →' : 'Create Patient Account →'}</span>
                  )}
                </button>
              </form>
                ...
                <p className="text-xs text-slate-500">
                  ...
                  <button 
                    ...
                    onClick={() => {
                      ...
                      updateSpeech('user', mode === 'signin' ? 'Setting up your secure patient record.' : 'Welcome back. Sign in to your portal.');
                    }}
                  >
          ...
          <section className="lg:col-span-6 bg-gradient-to-br from-[#1A5CFF] via-[#0b4ae2] to-[#072d9e] text-white p-7 sm:p-10 lg:p-11 flex flex-col justify-between relative overflow-hidden order-1 lg:order-2">
            ...
            <div className="relative z-10">
              <h1 className="text-3xl sm:text-4xl lg:text-[42px] font-heading font-bold tracking-tight text-white leading-[1.12]">
                Your health,<br />
                <span className="text-blue-200">securely managed.</span>
              </h1>
              <p className="mt-3 text-blue-100/90 text-sm sm:text-base leading-relaxed max-w-lg font-normal">
                Access your longitudinal patient record, connect with your care team, and track your health progress seamlessly.
              </p>
            </div>
              ...
              <div className="relative w-full max-w-[380px] flex items-center justify-center">
                ...
                <div className="absolute -top-3 -left-3 sm:-left-6 bg-white/95 backdrop-blur-md rounded-2xl p-2.5 sm:p-3 shadow-xl border border-white/80 flex items-center gap-2.5 float-slow z-20">
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-[10px] text-slate-500 font-medium">HIPAA Compliant</div>
                  </div>
                </div>
              </div>
            </div>
